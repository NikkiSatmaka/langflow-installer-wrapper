...
The installer applies `constraints.txt` with `uv pip install langflow --constraints=<path>`. On Windows machines whose user profile path contains a space (e.g. `C:\Users\John Doe\langflow`), uv reported the constraint file as not found: the path appeared truncated at the first space.

This is not a shell quoting problem. Windows PowerShell 5.1 (the runtime used by `Install Langflow.bat`) quotes native-command arguments containing spaces when it builds the command line, and argument array splatting preserves each element as a single token. The prior fixes (v1.9.1 separate-argument splat, the v1.9.2 quoted-path attempt, and the `--constraint=path` equals form) all assumed the argument was not reaching uv intact.

uv itself re-splits the value of `--constraints`/`-c`/`--override`/`--requirements`/`-r` on whitespace (pip's `PIP_CONSTRAINT` list semantics), then truncates a single path at the first space. This was reproduced against the project's pinned uv 0.12.1:

```
$ uv pip install pytest --constraints="/tmp/opencode/uv space test/constraints file.txt"
...
- `install-langflow-script.ps1`: `Copy-Item $ConstraintsFile "$LangflowDir\constraints.txt" -Force`, then `uv pip install ... --constraints=constraints.txt` with CWD set to `$LangflowDir` (already pushed before install).
...
- The `--constraints=constraints.txt` wording in AGENTS.md and the smoke test instructions remains accurate; only the mechanism for staging the file changed.
