...
Provide a single-click (or double-click) solution for users to install, run, and uninstall Langflow (`==1.11.6`) using `uv` as the package manager on Windows, macOS, and Linux — with no administrative privileges required on any platform.
...
| langflow==1.11.6 yanked on PyPI | Catch the pip error and retry with the version pin stripped from `requirements.txt` |
