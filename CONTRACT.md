...
Provide a single-click (or double-click) solution for users to install, run, and uninstall Langflow (`==1.11.2`) using `uv` as the package manager on Windows, macOS, and Linux — with no administrative privileges required on any platform.
...
4. Install Langflow: `uv pip install langflow==1.11.2`.
...
| langflow==1.11.2 yanked on PyPI | Catch the pip error and suggest removing the version pin |
