...
**Constraints file**:
A `constraints.txt` shipped alongside the installer scripts that constrains only the transitive dependencies known to break on certain platforms by shipping source-only releases without pre-built wheels. Currently empty — every dependency ships pre-built wheels on all target platforms. Applied via the `--constraints=` flag at install time.
...
**Stable release**:
A tag `v<LangflowVersion>` (e.g., `v1.11.3`) that pins that exact Langflow version in the installer scripts. The tag-triggered CI runs the full OS-matrix install test with the pinned version before packaging and publishing.

**Constraint-fix release**:
A `v<LangflowVersion>-N` postfix when constraints need updating between Langflow versions (e.g., `v1.11.3-1`).

**Langflow version**:
The upstream Langflow package version pinned in the installer scripts (e.g., `1.11.3`). Changed manually when a new Langflow version is tested and released.
