...
**Constraints file**:
A `constraints.txt` shipped alongside the installer scripts that constrains only the transitive dependencies known to break on certain platforms by shipping source-only releases without pre-built wheels. Currently empty — every dependency ships pre-built wheels on all target platforms. Applied via the `--constraint=` flag at install time.
